...
test('学生和成绩操作区可仅用键盘抵达，桌面视口中的提交按钮稳定可见', async ({ page }) => {
  ...
  const gradeEntry = page.getByRole('button', { name: /键盘学生.*1 条成绩|键盘学生.*0 条成绩/ })
  await gradeEntry.focus()
  await page.keyboard.press('Enter')
  const profile = page.getByRole('dialog', { name: '键盘学生' })
  const gradeTab = profile.getByRole('tab', { name: '成绩' })
  await gradeTab.focus()
  await page.keyboard.press('Enter')
  const gradeForm = profile.getByRole('tabpanel', { name: '键盘学生 的成绩' }).locator('form')
  const subject = gradeForm.getByRole('textbox', { name: '档案成绩学科' })
  await subject.focus()
